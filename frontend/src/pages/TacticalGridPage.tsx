    ...
    <div className="flex flex-col h-[calc(100vh-6.5rem)] space-y-3 font-sans">
      ...
      >
        {Array.from({ length: selectedGrid.count }).map((_, slotIdx) => {
          ...
          return (
            <TacticalGridSlotCard
              key={slotIdx}
              slotIdx={slotIdx}
              cam={cam}
              isExpanded={isExpanded}
              speedInfo={speedInfo}
              slotVision={slotVision}
              mutedAll={mutedAll}
              filteredCameras={filteredCameras}
              onCycleVision={cycleSlotVision}
              onSelectCamera={setCameraForSlot}
              onToggleExpand={(idx) => setExpandedIndex(expandedIndex === idx ? null : idx)}
              getVisionFilter={getVisionFilter}
            />
          );
        })}
      </div>
...
interface TacticalGridSlotCardProps {
  slotIdx: number;
  cam?: Camera;
  isExpanded: boolean;
  speedInfo?: { speed: number; alert: boolean };
  slotVision: VisionMode;
  mutedAll: boolean;
  filteredCameras: Camera[];
  onCycleVision: (slotIdx: number) => void;
  onSelectCamera: (slotIdx: number, camId: string) => void;
  onToggleExpand: (slotIdx: number) => void;
  getVisionFilter: (mode: VisionMode) => string;
}

const TacticalGridSlotCard: React.FC<TacticalGridSlotCardProps> = ({
  slotIdx,
  cam,
  isExpanded,
  speedInfo,
    ...
    >
      ...
      <div className="px-3 py-2 bg-[#0D1522] border-b border-[#1A2638] flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <select
            value={cam?.camera_id || ''}
            onChange={(e) => onSelectCamera(slotIdx, e.target.value)}
          ...
          {/* Expand / Minimize */}
          <button
            onClick={() => onToggleExpand(slotIdx)}
      ...
      {/* Video Player Area with Active Vision Filter & Dynamic Tracking Overlay */}
      <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden select-none">
        ...
        {/* Dynamic Object Tracking Overlay with Trajectories & Plates */}
        {cam && (
        ...
        {slotVision === 'night_vision' && (
          <>
            ...
            <div className="absolute top-2 right-2 pointer-events-none bg-black/80 border border-emerald-500/50 px-2 py-0.5 rounded text-[9px] font-mono text-emerald-300 font-bold flex items-center gap-1 shadow-sm z-30">
        ...
        {slotVision === 'flir_thermal' && (
          <>
            ...
            <div className="absolute right-2 top-8 bottom-8 w-2.5 rounded-md bg-gradient-to-t from-indigo-900 via-blue-600 via-purple-600 via-rose-500 via-amber-400 to-white pointer-events-none opacity-85 border border-white/30 shadow-md flex flex-col justify-between items-center text-[7px] font-mono text-white font-black py-0.5 z-30">
              <span>85°</span>
              <span>50°</span>
              <span>15°</span>
            </div>
            <div className="absolute top-2 right-2 pointer-events-none bg-black/80 border border-amber-500/50 px-2 py-0.5 rounded text-[9px] font-mono text-amber-300 font-bold flex items-center gap-1 shadow-sm z-30">
        ...
        {slotVision === 'edge_cv' && (
          <>
            <div className="absolute top-2 right-2 pointer-events-none bg-black/80 border border-cyan-500/50 px-2 py-0.5 rounded text-[9px] font-mono text-cyan-300 font-bold flex items-center gap-1 shadow-sm z-30">
        ...
        <div className="absolute top-2 left-2 pointer-events-none flex flex-col gap-1 z-30">
        ...
        <div className="absolute bottom-2 right-2 pointer-events-none flex items-center gap-1.5 z-30">
        ...
        {/* Corner Crosshairs */}
        <div className="absolute top-2 left-2 w-3 h-3 border-t-2 border-l-2 border-cyan-400 pointer-events-none opacity-60 z-30" />
        <div className="absolute top-2 right-2 w-3 h-3 border-t-2 border-r-2 border-cyan-400 pointer-events-none opacity-60 z-30" />
        <div className="absolute bottom-2 left-2 w-3 h-3 border-b-2 border-l-2 border-cyan-400 pointer-events-none opacity-60 z-30" />
        <div className="absolute bottom-2 right-2 w-3 h-3 border-b-2 border-r-2 border-cyan-400 pointer-events-none opacity-60 z-30" />
      </div>
